...
\begin{quote}
\small\itshape
Note on what is measured versus pending. Every table in this chapter reports
real measurements from the deployed system (Whisper \texttt{small} + Qwen2.5-7B-Instruct,
CPU-only reference machine, \texttt{temperature}\,$=0$), obtained by running
...
\textbf{ablation} tables (Whisper size, LLM size, few-shot vs.\ zero-shot) remain
labelled placeholders (\result{XX.X}): each requires restarting the containers
with a different model and re-running the full corpus, which was out of scope
for this pass and is left as an immediate next step, not a methodological gap.
\end{quote}
...
(Section~4.2.1): the category taxonomy grew from seventeen to twenty-nine
...
Section~5.6 remain representative of the system's behavior; only the
category-taxonomy size referenced in Section~5.6.2 reflects the
...
immediate next step, alongside the ablation studies.
...
0.80 target for category. Section~5.6 shows these are not diffuse, unexplained
...
language gap. Section~5.6 traces this precisely to how Qwen2.5-7B-Instruct
...
currencies (Table~4.3, chosen to cover the Arab region plus major reserve
...
coverage, not a model failure (Section~5.6). The other systematic pattern in
this pilot is the same category-defaulting behavior seen at full scale
(Section~5.6.2): both purchases in the multi-transaction clip (an iPhone and a
...
in Section~9.3, and for the faster-whisper upgrade path in Chapter~6.

\section{Ablation Studies (Planned, Not Yet Run)}

The ablation studies below quantify the contribution of each design choice.
Each requires restarting the containers with a different model configuration
and re-running the full 274-utterance corpus (roughly 30 CPU-minutes per
configuration); they were not executed in this pass and are reported as
labelled placeholders so the thesis structure and methodology are complete and
ready to receive the numbers.

\subsection{Ablation 1 --- Whisper Model Size}

\begin{table}[H]
\centering
\caption{Ablation 1 (planned): Whisper model size versus WER and latency.}
\label{tab:abl-whisper}
\small
\renewcommand{\arraystretch}{1.3}
\begin{tabularx}{\textwidth}{X c c c c}
\toprule
\textbf{Model} & \textbf{WER ar (\%)} & \textbf{WER en (\%)} & \textbf{WER mixed (\%)} & \textbf{Latency (ms)} \\
\midrule
\texttt{base}   & \result{XX.X} & \result{XX.X} & \result{XX.X} & \result{XXXX} \\
\texttt{small}  & 76.0 (pilot, $n$=4) & 17.4 (pilot, $n$=2) & \result{XX.X} & 3{,}890 \\
\texttt{medium} & \result{XX.X} & \result{XX.X} & \result{XX.X} & \result{XXXX} \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{table}[H]
\centering
\caption{Ablation 2 (planned): language-model size versus extraction quality (condition (a)).}
\label{tab:abl-llm}
\small
\renewcommand{\arraystretch}{1.3}
\begin{tabularx}{\textwidth}{X c c c c}
\toprule
\textbf{Model} & \textbf{amount} & \textbf{type} & \textbf{category F\textsubscript{1}} & \textbf{Latency (ms)} \\
\midrule
\texttt{qwen2.5:3b-instruct} & \result{XX.X} & \result{XX.X} & \result{0.XX} & \result{XXXX} \\
\texttt{qwen2.5:7b-instruct} & 65.6 & 99.3 & 0.554 & 6{,}995 \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{table}[H]
\centering
\caption{Ablation 3 (planned): few-shot versus zero-shot prompting (condition (a), Qwen2.5-7B).}
\label{tab:abl-fewshot}
\small
\renewcommand{\arraystretch}{1.3}
\begin{tabularx}{\textwidth}{X c c c}
\toprule
\textbf{Prompt} & \textbf{amount} & \textbf{category F\textsubscript{1}} & \textbf{count acc.} \\
\midrule
Zero-shot (system only) & \result{XX.X} & \result{0.XX} & \result{XX.X} \\
Few-shot (8 examples)   & 65.6 & 0.554 & 100.0 \\
\bottomrule
\end{tabularx}
\end{table}
...
illustrating the two systematic error modes analyzed in Section~5.6 alongside
...
error also explains part of the low \texttt{name} score (Section~5.6.3): when
...
own risk register anticipated (Section~1.7, ``Qwen mislabels
category/currency/amount on rare phrasings''), and it is the strongest
...
(Appendix~A.3), this is exactly the set of categories for which the prompt
...
compound-number failure (Section~5.6.2) rather than independent naming errors.
...
the Arab region plus major reserve currencies; Table~4.3). No prediction could
...
before it reaches the database (Section~3.2, two-step save). The measured
...
identified in Sections 5.6.1--5.6.2, are the highest-priority items in the
